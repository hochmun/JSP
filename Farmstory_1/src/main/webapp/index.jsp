...
<%
	List<BoardArticleDTO> badtos = BoardArticleDAO.getInstance().selectLatests("grow", "school", "story");
%>
<script>
	$(function(){
		// 공지사항 최신글 가져오기
		// 기본으로 보여주는 탭
		$.get('/Farmstory_1/Board/proc/getLatests.jsp?cate=notice',function(data){
			for(let latest of data) {
				let url = "/Farmstory_1/Board/view.jsp?group=community&cate=notice&pg=1&no="+latest.no;
				$('#tabs-1 > .txt').append("<li><a href='"+url+"'>"+latest.title+"</a></li>");
			}
		});
		// 각 탭 클릭시 데이터 받기
		$(document).one('click', '#tabs-2-click', function(){
			$.get('/Farmstory_1/Board/proc/getLatests.jsp?cate=qna',function(data){
				for(let latest of data) {
					let url = "/Farmstory_1/Board/view.jsp?group=community&cate=qna&pg=1&no="+latest.no;
		...
		$(document).one('click', '#tabs-3-click', function(){
			$.get('/Farmstory_1/Board/proc/getLatests.jsp?cate=faq',function(data){
				for(let latest of data) {
					let url = "/Farmstory_1/Board/view.jsp?group=community&cate=faq&pg=1&no="+latest.no;
...
<main>
    ...
    <div class="quick">
        <a href="./Board/list.jsp?group=community&cate=menu">
            <img src="./img/main_banner_sub1_tit.png" 
            alt="오늘의 식단">
        </a>
        <a href="./Board/list.jsp?group=community&cate=chef">
    ...
    <div class="latest">
        <div>
            <a href="./Board/list.jsp?group=croptalk&cate=grow">
                <img src="./img/main_latest1_tit.png" alt="텃밭 가꾸기">
            </a>
            <img src="./img/main_latest1_img.jpg" alt="이미지">
            <table>
            	<% for(int i = 0; i < badtos.size(); i++) { %>
            		<% if(badtos.get(i).getCate().equals("grow")) { %>
		                <tr>
		                    <td>></td>
		                    <td>
		                        <a href="/Farmstory_1/Board/view.jsp?group=croptalk&cate=grow&pg=1&no=<%=badtos.get(i).getNo()%>"><%= badtos.get(i).getTitle() %></a>
		                    </td>
		                    <td><%= badtos.get(i).getRdate() %></td>
		                </tr>
	                <% } %>
                <% } %>
            </table>
        ...
        <div>
            <a href="./Board/list.jsp?group=croptalk&cate=school">
                <img src="./img/main_latest2_tit.png" alt="귀농학교">
            </a>
            <img src="./img/main_latest2_img.jpg" alt="이미지">
            <table>
                <% for(int i = 0; i < badtos.size(); i++) { %>
            		<% if(badtos.get(i).getCate().equals("school")) { %>
		                <tr>
		                    <td>></td>
		                    <td>
		                        <a href="/Farmstory_1/Board/view.jsp?group=croptalk&cate=grow&pg=1&no=<%=badtos.get(i).getNo()%>"><%= badtos.get(i).getTitle() %></a>
        ...
        <div>
            <a href="./Board/list.jsp?group=croptalk&cate=story">
                <img src="./img/main_latest3_tit.png" alt="농작물이야기">
            </a>
            <img src="./img/main_latest3_img.jpg" alt="이미지">
            <table>
                <% for(int i = 0; i < badtos.size(); i++) { %>
            		<% if(badtos.get(i).getCate().equals("story")) { %>
		                <tr>
		                    <td>></td>
		                    <td>
		                        <a href="/Farmstory_1/Board/view.jsp?group=croptalk&cate=grow&pg=1&no=<%=badtos.get(i).getNo()%>"><%= badtos.get(i).getTitle() %></a>
    ...
    <div class="info">
        <div>
            ...
            <div class="btns">
                <a href="./Board/list.jsp?group=community&cate=qna">
                    <img src="./img/main_sub2_cs_bt1.png" alt="1:1 고객문의">
                </a>
                <a href="./Board/list.jsp?group=community&cate=faq">
